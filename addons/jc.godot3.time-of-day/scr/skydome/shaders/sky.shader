...
void opticalDepth(float y, out float sr, out float sm){
	...
	sr = zenith * RAYLEIGH_ZENITH_LENGTH * atm_rayleigh_level;
...
void fragment(){
	...
	vec2 mu = vec2(
		dot(sun_direction, worldPos), 
		dot(moon_direction, worldPos)
	);
	worldPos.y += horizon_level;
	
	// Atmospheric Scattering.
	float sr, sm;
	opticalDepth(worldPos.y, sr, sm);
	vec3 scattering = atmosphericScattering(sr, sm, mu.xy, v_angle_mult.xyz);
	...
	vec3 sunDisk = disk(worldPos, sun_direction, sun_disk_size) * 
		sun_disk_intensity * sun_disk_color.rgb * scattering;
